...
func NewFileBodySourceInDir(baseDir string, prefix string) (*FileBodySource, error) {
	...
	if errMkdir := privatefile.MkdirAll(baseDir); errMkdir != nil {
		return nil, errMkdir
	}
...
func (s *FileBodySource) CreatePart(prefix string) (*os.File, error) {
	...
	prefix = sanitizeTempPrefix(prefix)
	if errMkdir := privatefile.MkdirAll(s.dir); errMkdir != nil {
		return nil, errMkdir
	}
...
func (s *FileBodySource) AppendBytes(data []byte) error {
	...
	if len(s.paths) == 0 {
		...
		file, errOpen = privatefile.Open(s.paths[len(s.paths)-1], os.O_CREATE|os.O_WRONLY|os.O_APPEND)
	}
	if errOpen != nil {
		return errOpen
	}
