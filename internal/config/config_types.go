...
type RemoteManagement struct {
	...
	// DisableControlPanel skips serving and syncing the bundled management UI when true (default).
	DisableControlPanel bool `yaml:"disable-control-panel"`
	// DisableAutoUpdatePanel disables automatic periodic background updates of the management panel asset from GitHub (default).
	// When false, the background updater is enabled; when true, the panel is only downloaded on first access if missing.
